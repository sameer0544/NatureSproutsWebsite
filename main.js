...
document.addEventListener('DOMContentLoaded', () => {
    const waButton = document.getElementById('wa-launcher');
    const waChatBox = document.getElementById('wa-chat-box');
    
    if (waButton && waChatBox) {
        waButton.addEventListener('click', (e) => {
            e.preventDefault();
            if (waChatBox.classList.contains('active')) {
                waChatBox.classList.remove('active');
                setTimeout(() => { waChatBox.style.display = 'none'; }, 300);
            } else {
                waChatBox.style.display = 'flex';
                setTimeout(() => { waChatBox.classList.add('active'); }, 10);
            }
        });
        ...
        const sendMessage = () => {
            ...
            if (message) {
                ...
                url += "?text=Hi%20VistaIQ,%20I%20would%20like%20to%20request%20a%20demo.";
            }
